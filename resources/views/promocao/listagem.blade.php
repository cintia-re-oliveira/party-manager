...
@if(empty($promocoes))
	<div class="alert alert-danger">
		Você não tem nenhuma promoção cadastrada.
	</div>
@else
<div class="col-lg-8 col-lg-offset-2 text-center">
    <h2 class="margin-top-0 wow fadeIn">LISTA DE PROMOÇÕES</h2>
    <hr class="primary">
</div>
      ...
      <td> Remover </td>
		...
		@foreach ($promocoes as $p)
			<tr>
				...
				<td>{{ $p->valor }} </td>
				<td>{{ $p->descricao }} </td>
				<td align=center><a href="/promocoes/mostra/{{ $p->id }}"><i class="glyphicon glyphicon-list-alt"></i></a></td>
				<td align=center>
					<a href="{{action('PromocaoController@remove', $p->id)}}"><i class="glyphicon glyphicon-trash"></i></a>
				</td>
			</tr>
		@endforeach
	</table>
