...
<ul class="breadcrumb">
  ...
  <li class="active">Lista de Salão</li>
</ul>

@if(empty($salaos))
	<div class="alert alert-danger">
		Você não tem nenhum salão cadastrado.
	</div>
@else

<div class="col-lg-8 col-lg-offset-2 text-center">
    <h2 class="margin-top-0 wow fadeIn">LISTA DE SALÂO</h2>
    <hr class="primary">
</div>
      ...
      <td> Remover </td>
		...
		@foreach ($salaos as $p)
			<tr>
				...
				<td>{{ $p->facebook }} </td>
				<td>{{ $p->email }} </td>
				<td align=center><a href="/salao/mostra/{{ $p->id }}"><i class="glyphicon glyphicon-list-alt"></i></a></td>
				<td align=center>
					<a href="{{action('SalaoController@remove', $p->id)}}"><i class="glyphicon glyphicon-trash"></i></a>
				</td>
			</tr>
		@endforeach
	</table>
