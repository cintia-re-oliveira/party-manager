...
<ul class="breadcrumb">
  ...
  <li class="active">Lista de Fotos</li>
</ul>

@if(empty($fotos))
	<div class="alert alert-danger">
		Você não tem nenhuma foto cadastrada.
	</div>
@else
<div class="col-lg-8 col-lg-offset-2 text-center">
    <h2 class="margin-top-0 wow fadeIn">LISTA DE FOTOS</h2>
    <hr class="primary">
</div>
      ...
      <td> Remover </td>
		...
		@foreach ($fotos as $p)
			<tr class="{{$p->quantidade<=1 ? 'danger' : '' }}">
				<td>{{ $p->nome }} </td>
				<td align=center><a href="/fotos/mostra/{{ $p->id }}"><i class="glyphicon glyphicon-list-alt"></i></a></td>
				<td align=center>
					<a href="{{action('Fotoontroller@remove', $p->id)}}"><i class="glyphicon glyphicon-trash"></i></a>
				</td>
			</tr>
		@endforeach
	</table>
