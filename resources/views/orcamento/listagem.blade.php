...
@if(empty($orcamentos))
	<div class="alert alert-danger">
		Você não tem nenhum orçamento cadastrado.
	</div>
@else

<div class="col-lg-8 col-lg-offset-2 text-center">
    <h2 class="margin-top-0 wow fadeIn">LISTA DE ORÇAMENTOS</h2>
    <hr class="primary">
</div>
	...
	<table class="table table-striped table-bordered table-hover">
      ...
      <td> Remover </td>
		...
		@foreach ($orcamentos as $p)
			<tr>
				<td>{{ $p->nome }} </td>
				<td>{{ $p->telefone }} </td>
				<td>{{ $p->observacao }} </td>
				<td>{{ $p->datacadastro }} </td>
				<td align=center><a href="/orcamentos/mostra/{{ $p->id }}"><i class="glyphicon glyphicon-list-alt"></i></a></td>
				<td align=center>
					<a href="{{action('OrcamentoController@remove', $p->id)}}"><i class="glyphicon glyphicon-trash"></i></a>
				</td>
			</tr>
		@endforeach
	</table>
